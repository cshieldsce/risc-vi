...
The `+TEST=` plus-arg points at a `.mem` initialization file under [`test/mem/`](https://github.com/cshieldsce/riscv-5i/tree/main/test/mem). Drop in any RV32I program assembled to that format and the testbench runs it through the pipeline; the VCD writes to `waveform.vcd` for GTKWave.
...
The HTML report lands in `test/verification/riscof_work/report.html`. Plugins for the DUT and golden model live under [`test/verification/compliance/`](https://github.com/cshieldsce/riscv-5i/tree/main/test/verification/compliance) if you want to swap out targets.
...
From there, synthesize, implement, and program the board through the Vivado GUI or its TCL prompt. The PYNQ-Z2 top-level wiring (LEDs, clock, reset) lives in [`src/pynq_z2_top.sv`](https://github.com/cshieldsce/riscv-5i/blob/main/src/pynq_z2_top.sv); the synthesis and timing results are on the [FPGA]({{ '/fpga/' | relative_url }}) page.
